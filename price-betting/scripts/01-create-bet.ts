...
import { betSeed, openUntil, resolveDate, pricePrediction, wagerAmount, bonkUsdSwitchboardFeedDevnet, betCreator, admin, betProgram, connection } from "./99-config";
...
async function main() {
    ...
    try {
        //@ts-ignore
        const tx = await program.methods.createBet(betSeed, openUntil, resolveDate, pricePrediction, true, new PublicKey(bonkUsdSwitchboardFeedDevnet), wagerAmount).accountsPartial({
